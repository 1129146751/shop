...
import com.sineyun.commons.core.utils.WebUtil;
    ...
    public static Map<String, Claim> getClaims(String token) {
        try {
            ...
            if (exp < currentTimeMillis) {
                throw new CustomException( ResultCode.NOT_LOGIN,"会话已经过期,请重新登录!");
            }
            return claims;
        } catch (Exception exception) {
            log.error("非法参数异常,token={}", token);
            throw new CustomException( ResultCode.NOT_LOGIN,"非法参数异常,请重新登录!");
        }
    }
